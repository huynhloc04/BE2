...
class Recruiter:
    ...
    class Job:
        ...
        @staticmethod
        def jd_parsing(file: str):
            #   Save file as temporarily
            cleaned_filename = DatabaseService.clean_filename(file)
        ...
        def fill_job(data_form: schema.FillJob,
            ...
            if data_form.education:
                job_edus = [model.JobEducation(job_id=db_job.id, **json.loads(education[1:-1])) for education in data_form.education]
                db_session.add_all(job_edus)
            if data_form.language_certificates:
                lang_certs = [model.LanguageJobCertificate(job_id=db_job.id, **json.loads(lang_cert[1:-1])) for lang_cert in data_form.language_certificates]
                db_session.add_all(lang_certs)
            if data_form.other_certificates:
                other_certs = [model.OtherJobCertificate(job_id=db_job.id, **json.loads(other_cert[1:-1])) for other_cert in data_form.other_certificates]
        ...
        def get_detail_job(request, job_id, db_session):
            ...
            if job_result.status==schema.JobStatus.pending or job_result.status==schema.JobStatus.browsing:
                ...
                return {
                    #   Company
                    "logo":os.path.join(str(request.base_url), company_result.logo) if company_result.logo else None,
                    "company_name": company_result.company_name,
                    "industry": company_result.industry,
                    #   Job
                    "status": job_result.status,
    ...
    class Resume:
        ...
        @staticmethod
        def add_cart(data: schema.ResumeIndex, db_session: Session, current_user):
            cart_db = model.RecruitResumeCart(
                                        user_id=current_user.id,    #   Current recruiter's account
                                        resume_id=data.cv_id,
            )           
        ...
        @staticmethod
        def choose_resume_package(data: schema.ChooseResume, db_session: Session, current_user):
        ...
        def choose_interview(data: schema.InterviewForm, db_session: Session, background_task: BackgroundTasks, current_user):
            ...
            if data.interview_form.directly == schema.InterviewEnum.directly:
                ...
                <html>
                    <body>
                        <p> Dear {resume_result.ResumeVersion.name}, <br>
                            ...
                            - <a href="https://sharecv.vn/?page=accept&id={user.id}&token=&act=accept">"Accept"</a> nếu bạn đồng ý việc sự giới thiệu này. <br>
                            - <a href="https://sharecv.vn/?page=decline&id={user.id}&token=&act=decline">"Decline"</a> nếu bạn từ chối sự giới thiệu việc làm này. <br>
                ...
                #  Send mail
                Collaborator.Resume.send_email_request(data.cv_id, 
                                                       mail_contents, 
                                                       db_session, 
                                                       background_task, 
                                                       current_user)
...
class Admin:
    ...
    class Resume:
        ...
        def admin_review_matching(data: schema.AdminReviewMatching, db_session: Session):
            ...
            result.ResumeVersion.status = data.resume_status
            result.ResumeVersion.matching_decline_reason = data.decline_reason
            db.commit_rollback(db_session)
            
        
        @staticmethod
        def list_ai_matched(is_ai_matched: bool, db_session: Session):
            query = select(model.ResumeVersion).where(and_(model.ResumeVersion.is_lastest == True,
                                                            model.ResumeVersion.is_ai_matched == is_ai_matched))
            results = db_session.execute(query).scalars().all()
            if not results:
            ...
            return [{
                ...
                "industry": result.industry,
                "job_service": General.get_job_from_resume(result.cv_id, db_session).job_service,
                "referred_date": result.created_at,
            } for result in results]      
...
class Collaborator:
    ...
    class Job:    
        ...
        def get_detail_job(request, job_id, db_session):
            ...
            return {
                #   Company
                "company_logo": os.path.join(str(request.base_url), company_result.logo),
                "company_name": company_result.company_name,
                "company_cover_image": os.path.join(str(request.base_url), company_result.cover_image),
                "industry": company_result.industry,
                #   Job
                "status": job_result.status,
                "job_service": job_result.job_service,
        ...
        def list_job(request, job_status, db_session, current_user):
            ...
            if job_status == schema.CollaborateJobStatus.referred:
                ...
                return [{
                    "company_id": result[0],
                    "logo": os.path.join(str(request.base_url), result[1]),
    ...
    class Resume:    
        ...
        @staticmethod
        def update_draft(data: schema.UpdateResumeDraft, db_session: Session):
            result = General.get_detail_resume_by_id(data.cv_id, db_session)    
            if not result:
                raise HTTPException(status_code=404, detail="Resume doesn't exist!")
            
            #   Create draft
            result.ResumeVersion.is_draft = data.is_draft
            db.commit_rollback(db_session) 
        ...
        @staticmethod
        def get_detail_resume(request: Request, cv_id: int, db_session: Session):
            ...
            if not job_result:
                raise HTTPException(status_code=404, detail="Job doesn't exist!")
            valuate_result = General.get_resume_valuate(cv_id, db_session)
            ...
            return {
                ...
                "industry": resume_result.ResumeVersion.industry,
                "birthday": resume_result.ResumeVersion.birthday,
                "level": resume_result.ResumeVersion.level,
                "gender": resume_result.ResumeVersion.gender,
                "objectives": resume_result.ResumeVersion.objectives,
                "email": resume_result.ResumeVersion.email,
                "phone": resume_result.ResumeVersion.phone,
                ...
                "instagram": resume_result.ResumeVersion.instagram,
                "skills": resume_result.ResumeVersion.skills,
                "total_point": valuate_result.total_point,
                "experience": [{
                    "job_title": exper.job_title,
                    "company_name": exper.company_name,
                    "start_time": exper.start_time,
